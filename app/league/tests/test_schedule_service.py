...
class ScheduleApiTests2(TestCase):
    ...
    def setUp(self):
        ...
        self.team1 = create_team(self.league)
        self.team2 = create_team(self.league)
        self.teamSeason1 = create_team_season(self.team1, self.season)
        self.teamSeason2 = create_team_season(self.team2, self.season)
    ...
    def test_generate_schedule_and_validate_response(self):
        ...
        generate_response = self.client.post(
            generate_schedule_url(schedule.id))
        ...
        fetch_response = self.client.get(
            schedule_url(self.league.id, self.season.id))

        self.assertEqual(fetch_response.status_code, 200)
        schedule_data = fetch_response.data

        self.assertEqual(schedule_data["id"], schedule.id)
        self.assertEqual(schedule_data["start_date"], "2024-10-01")
        self.assertEqual(schedule_data["num_weeks"], 4)
        self.assertEqual(schedule_data["default_start_time"], "19:00:00")

        matchnights = schedule_data.get("matchnights", [])
        self.assertGreaterEqual(len(matchnights), 4)
